      ...
      { y: 80, opacity: 0, scale: 0.95 },
      {
        ...
        scrollTrigger: {
          ...
          onEnter: () => {
            ...
            if (!cardsRef.current) return;
            const plusBtns = cardsRef.current.querySelectorAll(".plus-btn");
            gsap.fromTo(
              plusBtns,
              { scale: 1 },
              {
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div ref={titleRef} className="text-center mb-16">
          ...
          <div className="mt-4 inline-flex items-center gap-2 bg-[#25D366]/10 border border-[#25D366]/20 rounded-full px-5 py-2">
            ...
            <span className="text-[#25D366] text-sm font-medium">{dict.menu.orderHint}</span>
          </div>
        </div>
        ...
        >
          {dict.menu.items.map(
            (
              ...
              <div
                key={index}
                className={`menu-card group relative bg-blanko-card border rounded-2xl p-6 overflow-hidden transition-all duration-300 ${
                  quantities[index] > 0
                    ? "border-blanko-yellow/40 shadow-[0_0_30px_rgba(245,166,35,0.1)]"
                    : "border-blanko-card-border"
                }`}
              >
                {/* Hover glow */}
                ...
                <div className="relative z-10 flex items-center justify-between mt-auto">
                  <div className="inline-flex items-center bg-blanko-yellow/10 border border-blanko-yellow/20 rounded-full px-3 py-1">
                  ...
                  <div className="flex items-center gap-1">
                    {quantities[index] > 0 && (
                      <>
                        <button
                          ...
                          className="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 border border-white/10 text-white hover:bg-red-500/20 hover:border-red-500/30 transition-all duration-200 text-sm font-bold"
                        >
                          -
                        </button>
                        <span className="w-8 text-center text-blanko-yellow font-bold text-sm">
                          {quantities[index]}
                        </span>
                      </>
                    )}
                    <button
                      onClick={() => onUpdateQty(index, 1)}
                      className="plus-btn w-8 h-8 flex items-center justify-center rounded-full bg-blanko-yellow/10 border border-blanko-yellow/30 text-blanko-yellow hover:bg-blanko-yellow/20 transition-all duration-200 text-sm font-bold"
                    >
                      +
                    </button>
                  </div>
                ...
                {/* Bottom accent line */}
                <div className={`absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-blanko-yellow/30 to-transparent transition-transform duration-500 ${
                  quantities[index] > 0 ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"
                }`} />
              </div>
            )
          )}
        ...
        {/* WhatsApp Order Button — desktop only (inline) */}
        {totalItems > 0 && (
          <div className="mt-10 hidden md:flex justify-center">
            <button
              onClick={onSendOrder}
              className="flex items-center gap-3 bg-[#25D366] text-white px-8 py-4 rounded-full text-lg font-bold hover:bg-[#1fb855] transition-all duration-300 hover:scale-105 shadow-[0_4px_30px_rgba(37,211,102,0.3)] hover:shadow-[0_6px_40px_rgba(37,211,102,0.4)]"
            >
              <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
              </svg>
              {dict.order.orderOnWhatsapp} — {totalItems} ({totalPrice.toFixed(2)}€)
            </button>
          </div>
        )}
...
export default function HomePage({
  ...
  // Order state (lifted from MenuSection so sticky bar can render outside scroll wrapper)
  const [quantities, setQuantities] = useState<number[]>(
    () => new Array(dict.menu.items.length).fill(0)
  );
  ...
  const totalPrice = quantities.reduce(
    (sum, qty, i) =>
      sum + qty * parseFloat(dict.menu.items[i].price),
    0
  );

  const sendWhatsAppOrder = () => {
    const lines = dict.menu.items
      .map(
        (item: { name: string; price: string }, i: number) =>
          quantities[i] > 0
            ? `${quantities[i]}x ${item.name} \u2014 ${(quantities[i] * parseFloat(item.price)).toFixed(2)}\u20AC`
            : null
      )
      .filter(Boolean);

    const message = [
      `\u{1F44B} ${dict.order.greeting}`,
      "",
      ...lines,
      "",
      `\u{1F4B0} ${dict.order.totalLabel}: ${totalPrice.toFixed(2)}\u20AC`,
      "",
      `${dict.order.thanks} \u{1F60A}`,
    ].join("\n");

    window.open(
      `https://wa.me/38343700217?text=${encodeURIComponent(message)}`,
      "_blank"
    );
  };
  ...
  return (
    <>
      ...
      {/* WhatsApp floating button — hidden on mobile when order bar is active */}
      <a
      ...
      {/* Sticky mobile order bar — fixed at bottom on mobile */}
      {totalItems > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-blanko-dark/95 backdrop-blur-xl border-t border-[#25D366]/20 px-4 py-3 safe-area-bottom">
          <button
            onClick={sendWhatsAppOrder}
            className="w-full flex items-center justify-center gap-3 bg-[#25D366] text-white py-3.5 rounded-full text-base font-bold shadow-[0_-4px_20px_rgba(37,211,102,0.2)]"
          >
